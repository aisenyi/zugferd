...
from zugferd.utils.zugferd.zugferd_xml import create_zugferd_xml
from facturx import generate_from_binary, get_facturx_xml_from_pdf, check_facturx_xsd, generate_facturx_from_file
...
def create_zugferd_pdf(docname, verify=True, format=None, doc=None, doctype="Sales Invoice", no_letterhead=0):
	...
	try:
		...
		if xml: 
			...
			# Save the file and add it as attachemnt to document
			file_url = f'/private/files/{docname.replace(" ", "-").replace("/", "-")}.pdf'
			with open(f'{frappe.local.site}/{file_url}', 'wb') as f:
				f.write(facturx_pdf)
